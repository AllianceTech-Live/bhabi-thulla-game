-- Always re-pool incomplete MM seats into the fullest open table.
-- Fixes: one browser on MMCLZ (3/4) while another stays stuck on MMYMQ.
...
begin
  ...
  -- Serialize matchmaking per game type
  perform pg_advisory_xact_lock(hashtext('bhabi-mm-' || v_type));

  -- Always leave incomplete MM lobbies so every Quick Match merges to one table.
  for r in
    select g.id as gid
    from public.games g
    join public.game_players me
      on me.game_id = g.id and me.player_id = v_user
  ...
  loop
    ...
    if not exists (
      select 1 from public.game_players where game_id = r.gid
    ) then
      update public.games
  ...
  -- Join the fullest open table that still has a free seat.
  ...
  -- Nobody waiting — open one new public table.
